...
export const createWorkItem = (projectId: string, data: { status_id: string; title?: string; description?: string; parent_id?: string }) =>
...
export const deleteWorkItem = (id: string, deleteFiles?: boolean) =>
  apiDelete(`/work-items/${id}${deleteFiles ? '?delete_files=true' : ''}`);
